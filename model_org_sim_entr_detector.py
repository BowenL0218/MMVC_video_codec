...
#     def forward(self, prev1, prev2, cur):
    def forward(self, prev1, prev2, prev3, prev4, cur, residuals_list, dir_image, diff, num_unchanged_total, num_total_blocks):
...
#         plt.imsave('Right' + ".png", sample_test_float)
        ...
        total_blocks = height_latent*width_latent
        count = 0
        # print(diff.shape)
        for i in range(height//8):
            for j in range(width//8):
                diff_blocks = torch.sum(diff[:,:,i*8:(i+1)*8,j*8:(j+1)*8])
                if diff_blocks != 0:
                    latent_diff[:,:,i,j] = 1
                    count += 1
        num_unchanged_frame = total_blocks - count
        # print(num_unchanged_frame)
        # print(torch.min(cur))
...
#         cur_pred = cur_pred[0][0]
        ...
        residual_org = residual.clone()
        residual_org = residual_org[latent_diff == 0]
        residual_org[residual_org != 0] = 1
        print(torch.sum(residual_org))
        # print(residual_org_ind.shape)
        # residual_org_mask[residual_org_mask != 0] = 1
        # residual_org_mask[residual_org_mask == 0] = 0
        # residual_org[residual_org != 0] = 
        residual *= latent_diff
        # print(torch.sum(residual_org_mask - latent_diff))
...
#         plt.savefig('hist_recon_clip.png')
        ...
        return clipped_recon_cur, mse_loss, bpp_feature, bpp_z, bpp, num_unchanged_frame, total_blocks
        # return clipped_recon_cur, bpp_feature, bpp_z, bpp
