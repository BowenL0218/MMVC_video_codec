...
from model_rev_raft_new_mod import *
...
import time
import random
from CLIC_dataset import CLICDataset
...
parser.add_argument('-n', '--name', default='raft_convLSTM_2_layers', help='experiment name')
parser.add_argument('-p', '--pretrain', default='/home/bowenliu/RAFT_video_compression_rev/logs/raft_convLSTM_2_layers/iter_124170.pth.tar', help='load pretrain model')
...
parser.add_argument('--config', dest='config', default = '/home/bowenliu/RAFT_video_compression_rev/configs/config_raft_convLSTM.json', required=False, help='hyperparameter in json format')
parser.add_argument('--seed', default=234, type=int, help='seed for random functions, and network initialization')
parser.add_argument('--dataset', dest='dataset', default = '/z/mrakeshc/clic2022/', required=False, help='the path of training dataset')
...
def test(step):
    with torch.no_grad():
        ...
        sumMsssimDB = 0
        cnt = 0
        for batch_idx, (prev1, prev2, prev3, prev4, prev5, cur) in enumerate(test_loader):
            batch_size, channels, H, W = prev1.shape
#             recon_cur, probs_features, prob_z = net(prev1.to('cuda'), prev2.to('cuda'), cur.to('cuda'))
            recon_cur, probs_features, prob_z = net(prev1.to('cuda'), prev2.to('cuda'), prev3.to('cuda'), prev4.to('cuda'), prev5.to('cuda'), cur.to('cuda'))
            ...
            save_path = '/home/bowenliu/recon_raft_convLSTM_2_layers/'
        ...
        logger.info("Test on CLIC dataset (seq 2): model-{}".format(step))
        sumBpp /= cnt
        sumPsnr /= cnt
        sumMsssim /= cnt
        sumMsssimDB /= cnt
...
def train(epoch, global_step):
    ...
    for batch_idx, (prev1, prev2, prev3, prev4, prev5, cur) in enumerate(train_loader):
        batch_size, seq, channels, H, W = prev1.shape
        start_time = time.time()
        global_step += 1
        recon_cur, probs_features, prob_z = net(prev1.reshape(-1, channels, H, W).to('cuda'), prev2.reshape(-1, channels, H, W).to('cuda'), prev3.reshape(-1, channels, H, W).to('cuda'), prev4.reshape(-1, channels, H, W).to('cuda'), prev5.reshape(-1, channels, H, W).to('cuda'), cur.reshape(-1, channels, H, W).to('cuda'))
        clipped_recon_image, mse_loss, bpp_feature, bpp_z, bpp = PSNR_bit_rate(recon_cur, cur.reshape(-1, channels, H, W).to('cuda'), probs_features, prob_z)
        
        distribution_loss = bpp
        distortion = mse_loss
        rd_loss = train_lambda * distortion + distribution_loss
#         rd_loss = train_lambda * distortion + alpha*distribution_loss
#         print("This is the loss value at each iteration:", distortion)
        optimizer.zero_grad()
        rd_loss.backward()
...
if __name__ == "__main__":
    ...
    tb_logger = None
    save_path = os.path.join('/home/bowenliu/RAFT_video_compression_rev/logs/', args.name)
    if args.name != '':
        os.makedirs(save_path, exist_ok=True)
