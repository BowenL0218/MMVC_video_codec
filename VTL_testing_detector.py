...
from model_org_sim_entr_detector import *
import torch
...
#     force_cudnn_initialization()
    ...
    for i in range(1):
        ...
        torch_transform_rev_org = transforms.Compose([transforms.ToTensor()])
        torch_transform_rev_back = transforms.Compose([transforms.Resize(288, 352)])
...
#             net.train()
            for cur_idx in image_list[4:]:
                ...
                prev4_org = cur
    #             print(cur.shape)
                # clipped_recon_image, mse_loss, bpp_feature, bpp_z, bpp, unchanged_total_frame, total_frame = net(prev1.to('cuda'), prev2.to('cuda'), prev3.to('cuda'), prev4.to('cuda'), cur.to('cuda'), residuals, dir_image, diff.to('cuda'), num_unchanged_total, num_total_blocks)
                ...
                # num_total_blocks += total_frame
                # num_unchanged_total += unchanged_total_frame
