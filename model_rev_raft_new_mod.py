...
#         self.temporalPredictor = Temporal_predictor(in_channel=feature_channel*5, out_channel=feature_channel)
        ...
        self.temporalPredictor = ConvLSTM(feature_channel, [feature_channel]*2, (3, 3), 2, True, True, False)
...
#     def forward(self, prev1, prev2, cur):
    def forward(self, prev1, prev2, prev3, prev4, prev5, cur):
...
#         cur_pred_cpu = prev1[0].cpu().permute(1,2,0).clamp(0., 1.)
#         cur_pred_float = np.float32(cur_pred_cpu)
#         plt.imsave("image1.png", cur_pred_float)
        # out = np.float32(out)
...
#             print(quant_noise_z.shape)
        ...
        #     compressed_z = torch.round(z)
            compressed_z = torch.round(z*32)/32
        ...
        #     compressed_feature_renorm = torch.round(feature_renorm)
            compressed_feature_renorm = torch.round(feature_renorm*32)/32
...
#         clipped_recon_cur = recon_cur.clamp(0., 1.)
        ...
        def feature_probs_based_sigma(feature, sigma, mu):
            ...
            probs = gaussian.cdf(feature + 0.015625) - gaussian.cdf(feature - 0.015625)
#             total_bits = torch.sum(torch.clamp(-1.0 * torch.log(probs + 1e-10) / math.log(2.0), 0, 50))
            return probs

        def iclr18_estimate_bits_z(z):
            prob = self.bitEstimator_z(z + 0.015625) - self.bitEstimator_z(z - 0.015625)
